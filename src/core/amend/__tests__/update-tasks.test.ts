...
import { parseTasks, serializeTasks, calculateProgress, findNextTaskId } from '../update-tasks.js';
import type { ParsedTask, TaskItem } from '../types.js';
...
`;
  ...
  describe('serializeTasks', () => {
    ...
    it('should include added tasks', () => {
      const tasks: ParsedTask[] = [];
      const added: TaskItem[] = [
        { id: '2.1', description: 'New task' }
      ];

      const result = serializeTasks(tasks, { added });

      expect(result).toContain('- [ ] 2.1 New task');
    });
